...
vault write -format=json pki/issue/$ROLE_NAME common_name="$COMMON_NAME" > cert.json
...
kubectl create secret tls $K8S_SECRET_NAME --cert=cert.crt --key=cert.key --namespace=$NAMESPACE --dry-run=client -o yaml | kubectl apply -f -
